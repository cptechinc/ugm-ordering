...
/* =============================================================
	Display Functions
============================================================= */
	private static function displayCheckout($data) {
		$config = self::pw('config');
		$checkoutm = self::pw('modules')->get('Checkout');
		...
		$html .= $config->twig->render('checkout/checkout.twig', ['checkoutm' => $checkoutm, 'user' => self::pw('user'), 'qnotes' => $qnotes, 'cart' => $cart]);
		$html .= $config->twig->render('cart/notes/modal.twig', ['qnotes' => $qnotes]);
		return $html;
	}

	private static function displayResponses($data) {
		$checkoutm = self::pw('modules')->get('Checkout');
		$billing   = $checkoutm->get_billing();
	...
	private static function displayResponseCheckout($data) {
		$session = self::pw('session');
		$html = '';
