<?php
	...
	$page->addHook('Page(template=order-edit)::searchItemsUrl', function($event) {
		$event->return = $event->object->wire('pages')->get('template=items-search')->url;
	});
	$page->addHook('Page(template=order-edit)::itemsJsonUrl', function($event) {
		$event->return = $event->object->wire('pages')->get('template=items-json')->url;
	});
